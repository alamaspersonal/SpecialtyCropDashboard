...
and overwrites the UnifiedCropPrice table in Supabase.
...
def load_and_format_recent_slugs():
    """
    Load all CSV files from recent_slugs/ and format for UnifiedCropPrice.
    ...
    if not combined_unified.empty:
        ...
        if dupes > 0:
            print(f"\nRemoved {dupes} duplicate UnifiedCropPrice rows")
...
def main():
    ...
    unified_df = load_and_format_recent_slugs()
    ...
    if success:
        print("\n✔ All done! Supabase UnifiedCropPrice updated with recent_slugs data.")
    else:
